...
namespace BudgetOnline.UI.Controls
{
    public class FormBuilder : IBuilder
    {
        ...
        protected bool _noRowHeader;
        public FormBuilder NoRowHeader()
        {
            _noRowHeader = true;
            return this;
        }
        ...
        protected virtual string GetActionsContent()
        {
            if (_actionsContentBuilder != null)
                if (_noRowHeader)
                    return string.Format("<div class=\"form-group clearfix\"><div class=\"col-md-12\">{0}</div></div>", _actionsContentBuilder().ToHtmlString());
                else
